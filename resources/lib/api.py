...
class Api:
    ...
    def get_next_in_playlist(self, position):
        ...
        # formatting issues ("S-1E-1") in SkipIntro popup
    ...
    def play_addon_item(self):
        if self.data.get('play_url'):
            ...
            event(message=self.data.get('id'), data=self.data.get('play_info'), sender='skipintroprovider', encoding=self.encoding)
